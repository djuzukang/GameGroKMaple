...
def single_match(frame, template, threshold=0.95):
    ...
    :param threshold:   The minimum correlation coefficient for a match.
    ...
    if max_val < threshold:
        return (0, 0), (0, 0)  # Return default if no match
    w, h = template.shape[::-1]
    bottom_right = (top_left[0] + w, top_left[1] + h)
    return top_left, bottom_right

def multi_match(frame, template, threshold=0.9):  # Increased threshold for Nunu World
    """
